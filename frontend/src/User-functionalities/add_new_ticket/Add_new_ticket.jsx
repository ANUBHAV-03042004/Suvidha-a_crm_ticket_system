      ...
      try {
        ...
        if (!response.ok) {
          ...
          const errorText = contentType?.includes('application/json')
            ...
            : await response.text();
          console.error('Session check failed:', { status: response.status, error: errorText });
          setError('Please log in to add a ticket.');
          navigate('/login');
        }
      } catch (err) {
        console.error('Session check error:', { message: err.message, stack: err.stack });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (contentType?.includes('application/json')) {
          ...
          errorMessage = `Server returned invalid response: ${text.slice(0, 200)}`;
        }
      ...
      console.error('Error saving ticket:', { message: error.message, stack: error.stack });
